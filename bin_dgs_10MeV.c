...
// 2-D's for Energy
  ...
  hEhiCln_nodop = make2D ("EhiCln_nodop", LENSP, 0, LENSP + 1, 400, 1, 400 + 1);
  ...
  for (i = 1; i < NGE + 1; i++)
    {
      ...
      eXsum1[i] = new TH2F ((char *) str, (char *) "energy vs sum1 plot", 5000, 0, 20000, 1500, -3000, 1500);
...
//              printf ("  %lli %10.2f\n", DGSEvent[i].event_timestamp - 2, DGSEvent[i].cfd_2);
  ...
  /* special loop to show that AUX channels are there */
  /* 24/3/25 */
  ...
  if (Pars.CurEvNo <= 1000000)
  {
    for (i = 0; i < ng; i++)
      if (DGSEvent[i].tpe == GE)
        printf ("AUX__DGSEvent[%i].sum1 | sum2 = %i | %i | %i; \n", i, Pars.CurEvNo, DGSEvent[i].sum1, DGSEvent[i].sum2);
//	d1 = (DGSEvent[i].sum2 - DGSEvent[i].sum1) / Pars.dgs_MM;
	d1 = ( DGSEvent[i].sum1 - DGSEvent[i].sum2)/Pars.dgs_MM ;
//	d1 = (DGSEvent[i].sum2 - DGSEvent[i].sum1) / 50.;
	gsid = DGSEvent[i].tid;
        hEhiCln_nodop->Fill (d1, gsid+110);
	d1 = ( DGSEvent[i].sum2  - DGSEvent[i].sum1 )/Pars.dgs_MM ;
	hEhiCln_nodop->Fill (d1, gsid+220);
  };
